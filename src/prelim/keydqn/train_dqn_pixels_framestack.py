import gymnasium as gym
import minigrid
from minigrid.wrappers import RGBImgPartialObsWrapper, ImgObsWrapper
from stable_baselines3.common.vec_env import DummyVecEnv, VecFrameStack
from stable_baselines3 import DQN
import os
import sys
import time
import torch as th

# Directories
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODELS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(BASE_DIR))), "src", "models", "keydqn_framestack")
LOG_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(BASE_DIR))), "src", "logs", "keydqn_framestack")

ENV_ID = "MiniGrid-DoorKey-5x5-v0"
MODEL_NAME = "DQN_Pixels_DoorKey5x5_FrameStack"

def train():
    print(f"--- STARTING DQN PIXEL + FRAMESTACK TRAINING ({ENV_ID}) ---")
    os.makedirs(MODELS_DIR, exist_ok=True)
    os.makedirs(LOG_DIR, exist_ok=True)

    # delay creation for DummyVecEnv
    def make_env():
        env = gym.make(ENV_ID, render_mode="rgb_array")
        env = RGBImgPartialObsWrapper(env, tile_size=8)
        env = ImgObsWrapper(env)
        return env

    #  Vectorize and Stack Frames
    env = DummyVecEnv([make_env])
    # Stack 4 frames -> Input shape becomes (4*3, 56, 56) = (12, 56, 56)
    env = VecFrameStack(env, n_stack=4)

    print(f"Observation Space: {env.observation_space.shape}")

    # The policy automatically handles the larger input channels (12 instead of 3)
    model = DQN(
        "CnnPolicy",
        env,
        verbose=1,
        tensorboard_log=LOG_DIR,
        device="cuda",
        buffer_size=100000,
        learning_rate=1e-4,
        exploration_fraction=0.2, 
        exploration_final_eps=0.05,
        target_update_interval=1000,
        train_freq=4,
        gradient_steps=1,
    )

    print("Training for 500,000 steps...")
    model.learn(total_timesteps=500000, tb_log_name=MODEL_NAME)
    
    save_path = os.path.join(MODELS_DIR, MODEL_NAME)
    model.save(save_path)
    print(f"Model saved to {save_path}")
    env.close()

def watch(max_steps_per_episode=None):
    print(f"--- WATCHING DQN PIXEL + FRAMESTACK AGENT ({ENV_ID}) ---")
    
    model_path = os.path.join(MODELS_DIR, MODEL_NAME)
    if not os.path.exists(model_path + ".zip"):
        print(f"Model not found at {model_path}.zip")
        return

    # Create environment with correct wrappers
    def make_env():
        env = gym.make(ENV_ID, render_mode="human")
        env = RGBImgPartialObsWrapper(env, tile_size=8)
        env = ImgObsWrapper(env)
        return env

    env = DummyVecEnv([make_env])
    env = VecFrameStack(env, n_stack=4)

    model = DQN.load(model_path, env=env)

    for ep in range(20):
        obs = env.reset() # VecEnv reset returns only obs
        done = False
        total_reward = 0
        step_count = 0
        
        # VecEnv loop is slightly different
        while not done:
            action, _ = model.predict(obs, deterministic=True)
            obs, reward, done_array, _ = env.step(action)
            done = done_array[0] # Single env
            total_reward += reward[0]
            step_count += 1
            if max_steps_per_episode is not None and step_count >= max_steps_per_episode:
                done = True 
            time.sleep(0.05)
        print(f"Episode {ep+1}: Reward {total_reward:.4f} (Steps: {step_count})")
    
    env.close()

if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--watch":
        max_steps = int(sys.argv[2]) if len(sys.argv) > 2 else None
        watch(max_steps)
    else:
        train()
